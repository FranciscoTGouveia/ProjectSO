...
response_manager* reader_stc_response_manager(uint8_t code_pipe, char buffer[MAX_LINE]) {
...
list_manager_response* reader_stc_list_response(uint8_t code_pipe, char buffer[MAX_LINE]) {
    list_manager_response* list_response = my_malloc(sizeof(list_manager_response));
    list_response->code = code_pipe;
    size_t offset = sizeof(uint8_t);
    memcpy(&list_response->last, buffer + offset, sizeof(uint8_t));
    offset += sizeof(uint8_t);
    memcpy(list_response->box_name, buffer + offset, (sizeof(char)*MAX_BOX_NAME));
    offset += (sizeof(char)*MAX_BOX_NAME);
...
list_manager_request* reader_stc_list_request(uint8_t code_pipe, char buffer[MAX_LINE]) {
    list_manager_request* list_request = my_malloc(sizeof(list_manager_request));
    list_request->code = code_pipe;
    memcpy(list_request->pipe_name, buffer + sizeof(uint8_t), (sizeof(char)*MAX_PIPE_NAME));
    return list_request;
}
...
messages_pipe* reader_stc_message(uint8_t code_pipe, char buffer[MAX_LINE]) {
    ...
    memcpy(message_request->message,buffer + sizeof(uint8_t), (sizeof(char)*1024));
