...
request* reader_stc_request(uint8_t code_pipe, char buffer[MAX_LINE]) {
    ...
    long unsigned int offset = sizeof(uint8_t);
    ...
    printf("dentro do reader este e o box %s\n",request_pipe->box_name);
    return request_pipe;
}
...
list_manager_request* reader_stc_list_request(uint8_t code_pipe, char buffer[MAX_LINE]) {
    ...
    long unsigned int offset = sizeof(uint8_t);
    memcpy(list_request->pipe_name, buffer + offset, (sizeof(char)*MAX_PIPE_NAME));
    return list_request;
}
...
messages_pipe* reader_stc_message(uint8_t code_pipe, char buffer[MAX_LINE]) {
    ...
    long unsigned int offset = sizeof(uint8_t);
    memcpy(message_request->message,buffer + offset, (sizeof(char)*1024));
