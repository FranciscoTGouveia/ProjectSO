...
int pcq_create(pc_queue_t* queue, size_t capacity) {
    queue =(pc_queue_t*)malloc(sizeof(pc_queue_t)); // check malloc
...
int pcq_destroy(pc_queue_t *queue) {
    ...
    for (int i = queue->pcq_head; i <= queue->pcq_tail; i++) {
...
int pcq_enqueue(pc_queue_t *queue, void* elem) {
    ...
    pthread_mutex_lock(&queue->pcq_current_size);
    ...
    if (queue->pcq_tail >= 0) {
        for (int i = queue->pcq_tail + 1; i > queue->pcq_head; i--) {
